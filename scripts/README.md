...
All command examples below assume you are running them from the repository root.
...
### `refresh-card-tier-matrix.js` (Crypto Card Tier Companion)

Rebuilds `CRYPTO_CARDS_TIERS.md` from the public `ccompare.cards` bundle.
This preserves richer tier-level fields (issuance fee, ATM limits, conversion fees,
card form, etc.) without expanding the main `CRYPTO_CARDS.md` table.

By default it refreshes the currently tracked intake set:

- `bleap`, `deblock`, `spritz`, `pyra`, `cypher`, `rebind`, `uglycash`, `holyheld`, `krak`

#### Usage

```bash
# Refresh tracked intake keys and write CRYPTO_CARDS_TIERS.md
node scripts/refresh-card-tier-matrix.js

# Print generated markdown to stdout (no write)
node scripts/refresh-card-tier-matrix.js --dry-run

# Include all active cards from the upstream dataset
node scripts/refresh-card-tier-matrix.js --all
...
- The script writes `CRYPTO_CARDS_TIERS.md` only when tier data materially changes.
- A date-only change in the `Last Updated` footer is treated as a no-op to avoid churn commits.

#### Automation

GitHub Actions workflow: `.github/workflows/refresh-card-tier-matrix.yml`
...
table using verified pricing from `data/merchant_pricing.json`.
...
- `frontend/public/merchant-center.xml` (public feed URL)

**Note:** Keep `data/merchant_pricing.json` updated with official
provider pricing so the feed stays compliant. Files under `artifacts/`
...
Pricing sources and exclusions are documented in `MERCHANT_FEED.md`.
...
```bash
./scripts/extract_wallets_subtree.sh https://github.com/chimera-defi/walletradar.git
```

It prints the push command and verification steps. Pair this with
`EXTRACT_STUB_PLAN.md` and `STUB_README_TEMPLATE.md` when
...
```bash
python3 scripts/recompute_scores.py
```

### `run_naming_workflow.py` (Naming + Domain Validation)

Runs a strict naming workflow from `branding/CANDIDATES.txt`:
...
- writes `branding/NAMING_VALIDATION.md`
- writes `branding/naming-workflow-output.json`
...
python3 scripts/run_naming_workflow.py --write --pretty-json
...
Verifies ramp provider URLs from `RAMPS.md` using a direct fetch plus
`r.jina.ai` proxy as a fallback to detect bot challenges.

#### Usage

```bash
python3 scripts/verify-ramps.py
```

#### Output

- `artifacts/ramps-url-checks.json`
- `artifacts/ramps-url-checks.txt`

### `refresh-github-data.sh` (Software Wallets)

Queries GitHub for **EVM software wallet** activity.

Queries the GitHub API to get the latest commit dates and activity status for all tracked wallet repositories. The tracked repos are discovered from the public `SOFTWARE_WALLETS.md` table, so adding or removing rows there automatically changes the refresh set.
...
./scripts/refresh-github-data.sh --json

# Markdown table for documentation
./scripts/refresh-github-data.sh --markdown
```
...
```bash
# Basic usage - outputs to stdout
./scripts/refresh-hardware-wallet-data.sh

# JSON output for programmatic use
./scripts/refresh-hardware-wallet-data.sh --json

# Markdown table for documentation
./scripts/refresh-hardware-wallet-data.sh --markdown
```

The tracked repos are discovered from `HARDWARE_WALLETS.md`. The script only follows rows with a public GitHub firmware repo and dedupes shared repos such as the Trezor and BitBox product lines.
...
Add or update the row in `SOFTWARE_WALLETS.md`. If the GitHub column links to a public repo, the refresh script will pick it up automatically.

### For Hardware Wallets

Add or update the row in `HARDWARE_WALLETS.md`. Public firmware repos with `✅ Full` open source are discovered automatically; SDK-only and non-firmware links are skipped.
